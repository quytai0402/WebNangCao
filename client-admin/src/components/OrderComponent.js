...
import { FaSearch, FaFilter, FaCalendar, FaSort, FaEnvelope, FaFilePdf, FaTimes, FaUser, FaList, FaShoppingBag, FaEye, FaTrash, FaMoneyBillWave, FaCreditCard } from 'react-icons/fa';
...
class Order extends Component {
    ...
    loadOrders = async (page) => {
        ...
        try {
            ...
            toast.error('Lỗi tải danh sách đơn hàng: ' + error.message);
        } finally {
    ...
    handleViewOrderDetail = async (orderId) => {
        try {
            ...
            if (response.data.success) {
                // Lấy giá trị paymentMethod từ response
                const paymentMethod = response.data.order.paymentMethod || 'cash';

                // Đảm bảo dữ liệu khách hàng được lưu trữ đúng vị trí
                const orderData = response.data.order;

                // Kiểm tra cấu trúc dữ liệu khách hàng
                // Đơn hàng có thể lưu email trong customer hoặc customerInfo
                const customerData = orderData.customer || {};
                const customerInfo = orderData.customerInfo || {};

                const customer = {
                    name: customerData.name || customerInfo.name || 'Không có tên',
                    email: customerData.email || customerInfo.email || null,
                    phone: customerData.phone || customerInfo.phone || 'N/A',
                    address: customerData.address || customerInfo.address || 'N/A',
                    isRegistered: customerData.isRegistered || false
                };

                console.log("Thông tin khách hàng được xử lý:", customer);

                const order = {
                    ...orderData,
                    customer: customer,
                    customerInfo: customer, // Đảm bảo thông tin khách hàng được lưu ở cả hai vị trí
    ...
    renderTableBody() {
        ...
        return orders.map((order) => (
            <tr key={order._id}>
                ...
                <td>{formatCurrency(this.calculateOrderTotal(order.items))}</td>
    ...
    render() {
        const { showDetailModal, selectedOrder, updatingStatus, loading } = this.state;
                ...
                >
                    <Modal.Header closeButton className="modal-header-gradient-success">
                        <Modal.Title className="d-flex align-items-center text-white">
                            ...
                            Chi tiết đơn hàng <span className="order-id-label ms-2">#{selectedOrder?._id}</span>
                        </Modal.Title>
                    </Modal.Header>
                                    ...
                                    <Col md={6}>
                                        <div className="order-info p-4 bg-light h-100">
                                            ...
                                            <div className="info-group">
                                                ...
                                                <div className="info-item d-flex mb-3">
                                                    <div className="info-label text-muted">Tổng tiền:</div>
                    ...
                    <Modal.Footer>
                        <Button variant="secondary" onClick={() => this.setState({ showDetailModal: false })}>
